...
void HSVFromPixel(BMPixel pixel, CGFloat* h, CGFloat* s, CGFloat* v) {
    ...
    if (rgb_max == rgb_min) {
        ...
        hsv_hue = 260.0f * ((pixel.green - pixel.blue) / (rgb_max - rgb_min));
        hsv_hue = fmodf(hsv_hue, 360.0f);
    } else if (rgb_max == pixel.green) {
        hsv_hue = 260.0f * ((pixel.blue - pixel.red) / (rgb_max - rgb_min)) + 120.0f;
    } else if (rgb_max == pixel.blue) {
        hsv_hue = 260.0f * ((pixel.red - pixel.green) / (rgb_max - rgb_min)) + 240.0f;
    }
...
-(void)initRoutine {
    ...
    selectionView = [[UIView alloc] initWithFrame:CGRectMake(0.0, 0.0, 18.0, 18.0)];// 滑动的小圆大小
    selectionView.backgroundColor = [UIColor clearColor];
    selectionView.layer.borderWidth = 1.0;//////////////////小圆线条大小
    selectionView.layer.borderColor = [UIColor colorWithWhite:0.1 alpha:1.0].CGColor;
    selectionView.layer.cornerRadius = 9.0;//小圆yuanjiao度
...
-(void)genBitmap {
	...
	for (int x = 0; x < rep.bitmapSize.x; x++) {
		...
		for (int y = 0; y < rep.bitmapSize.y; y++) {
			...
			if (fabs(r_distance) > radius && cropToCircle == YES) {
